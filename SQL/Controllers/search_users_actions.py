...
def follow_user(conn, cursor, user, follower_id):
    if str(user.get_usr()) == follower_id:
        print("You cannot follow yourself!\n")
        return
    
    cursor.execute(follows_queries.check_follower, (int(follower_id), user.get_usr()))
    follower = cursor.fetchone()
    
    if follower != None:
        print(f"You are already following {int(follower_id)}\n")
    else:
        cursor.execute(follows_queries.follow_user, (user.get_usr(), int(follower_id), datetime.now().date().strftime("%Y-%m-%d")))
        conn.commit()
        print(f"You are now following {int(follower_id)}\n")
...
def see_user_details(conn, cursor, user):
    ...
    while True:
        ...
        if option == '1':
            follow_user(conn, cursor, user, input_user_id)
        elif option == '2':
            offset += 3
            cursor.execute(tweets_queries.three_recent_tweets, (int(input_user_id), offset))
